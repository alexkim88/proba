document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ua: {
            nav_home: 'Головна', nav_categories: 'Категорії',
            nav_products: 'Популярні товари',
        ...
        ru: {
            nav_home: 'Главная', nav_categories: 'Категории',
            nav_products: 'Популярные товары',
            ...
            sort_cheap: 'от дешевых к дорогим',
            sort_expensive: 'от дорогих к дешевым',
            sort_newest: 'новинки',
    ...
    function applyNavigationSettings() {
        ...
        if (!Array.isArray(data)) {
            ...
            [1, 2].forEach(coreId => {
                const defaultItem = DEFAULT_NAVIGATION.find(i => i.id === coreId);
                const existing = data.find(i => i.id === coreId);
                if (!existing && defaultItem) {
    ...
    let currentPage = 1;
    let itemsPerPage = 8;
    let currentLayoutView = 'grid'; // 'grid' or 'compact'
    ...
    if (isCategoryPage && catId) {
        ...
        if (catInfo) {
            setTimeout(() => {
                ...
                // Show resolution filter only for cameras
                const resFilter = document.getElementById('resolutionFilterBlock');
                if (resFilter) {
                    resFilter.style.display = (catId === 'cat_cameras') ? 'block' : 'none';
                }
            }, 50);
        }
    }
    ...
    function renderProducts() {
        ...
        // --- Custom Checkbox Filters Start ---
        // Status filtering
        if (window.currentStatusFilters && window.currentStatusFilters.length > 0) {
            products = products.filter(p => {
                // If inStock field doesn't exist, randomly assume true for preview unless price says otherwise
                let inStock = p.inStock;
                if (inStock === undefined) {
                    if (p.availability) {
                         inStock = p.availability.toLowerCase().includes('в наявності') || p.availability.toLowerCase().includes('в наличии');
                    } else {
                         inStock = true; 
                    }
                }
                const isItemInStock = (inStock === true || inStock === 'true');
                
                const showInStock = window.currentStatusFilters.includes('in_stock');
                const showOutOfStock = window.currentStatusFilters.includes('out_of_stock');
                
                if (showInStock && isItemInStock) return true;
                if (showOutOfStock && !isItemInStock) return true;
                return false;
            });
        }
        
        // Resolution filtering (cameras only)
        if (currentCategory === 'cat_cameras' && window.currentResolutionFilters && window.currentResolutionFilters.length > 0) {
            products = products.filter(p => {
                // Collect all possible text sources for searching the resolution string
                const fields = [
                    p.name, p.name_ua, p.name_ru,
                    p.description, p.description_ua, p.description_ru,
                    ...(p.specs || []), ...(p.specs_ua || []), ...(p.specs_ru || [])
                ];
                const textToSearch = fields.filter(f => f).join(' ').toLowerCase();
                
                return window.currentResolutionFilters.some(res => {
                    const val = res.replace('mp', '').trim();
                    // Check for various resolution string formats (UA/RU "мп", EN "mp", with/without spaces)
                    return textToSearch.includes(val + ' мп') || 
                           textToSearch.includes(val + 'мп') || 
                           textToSearch.includes(val + ' mp') || 
                           textToSearch.includes(val + 'mp');
                });
            });
        }
        // --- Custom Checkbox Filters End ---
        ...
        if (paginationContainer) {
            const currentLimitValue = (itemsPerPage === 'all') ? totalProducts : itemsPerPage;
            const startIndex = (currentPage - 1) * currentLimitValue;
            productsToRender = (itemsPerPage === 'all') ? products : products.slice(startIndex, startIndex + currentLimitValue);
        }

        // Apply Layout Class
        if (productsCarousel) {
            if (currentLayoutView === 'compact') {
                productsCarousel.classList.add('view-compact');
                productsCarousel.classList.remove('category-page-grid'); 
            } else {
                productsCarousel.classList.remove('view-compact');
                productsCarousel.classList.add('category-page-grid');
            }
        }
        ...
        productsToRender.forEach(p => {
            ...
            const badgeHtml = p.badge ? `<span class="card-badge badge-custom">${p.badge}</span>` : '';
            const discountPercent = p.discount || (p.oldPrice ? Math.round((1 - p.price / p.oldPrice) * 100) : 0);
            const discountHtml = (discountPercent > 0) ? `<span class="card-badge badge-sale">-${discountPercent}%</span>` : '';
            
            const badgesContainer = (badgeHtml || discountHtml) ? `
            ...
            article.innerHTML = `
                ${badgesContainer}
                <button class="card-wishlist-btn" title="Додати в обране">
                ...
                <div class="card-content">
                    ...
                    <div class="card-meta">
                        ...
                        <div class="card-availability">
                            ${currentLang === 'ua' ? 'Є в наявності' : 'Есть в наличии'}
                        </div>
                    </div>
                    <div class="card-footer">
                        <div class="price-block">
                            <span class="price-new" style="${p.oldPrice ? 'color: #f43f5e;' : ''}">${Number(p.price).toLocaleString('uk-UA')}₴</span>
                            ${p.oldPrice ? `<span class="price-old" style="text-decoration: line-through; color: #94a3b8; font-size: 0.85rem; margin-left: 8px;">${Number(p.oldPrice).toLocaleString('uk-UA')}₴</span>` : ''}
                        </div>
                    </div>
                    <button class="btn-one-click-card btn-buy-now-quick" data-product="${name}" data-price="${p.price}" data-image="${p.image}">
                        <i class="ph ph-shopping-bag"></i>
                        <span>${currentLang === 'ua' ? 'Купити в 1 клік' : 'Купить в 1 клик'}</span>
                    </button>
                </div>
                <button class="card-cart-btn-float add-to-cart" data-name="${name}" data-price="${p.price}" data-image="${p.image}" title="${btnBuyText}">
    ...
    function renderPagination(total) {
        ...
        const limitVal = (itemsPerPage === 'all') ? total : itemsPerPage;
    ...
    // =============================================
    // PRODUCTS SORTING (Desktop & Mobile)
    // =============================================
    const sortDropdowns = document.querySelectorAll('.sort-dropdown, .mobile-sort-dropdown');
    const currentSortLabel = document.getElementById('currentSort');
    const mobileSortValue = document.getElementById('mobileSortValue');

    sortDropdowns.forEach(dropdown => {
        const header = dropdown.querySelector('.sort-header, .mobile-control-btn');
        const list = dropdown.querySelector('.sort-list');
        
        if (header && list) {
            header.addEventListener('click', (e) => {
                e.stopPropagation();
                // Close other sort lists first
                document.querySelectorAll('.sort-list.active').forEach(l => {
            ...
            list.addEventListener('click', (e) => {
                ...
                // Sync all labels
                if (currentSortLabel) {
                    currentSortLabel.textContent = sortText;
                ...
                if (mobileSortValue) {
                    mobileSortValue.textContent = sortText;
                }
    ...
    function updateSortIcons() {
        const desktopIcon = document.getElementById('desktopSortIcon');
        const mobileIcon = document.getElementById('mobileSortIcon');
        const anyActive = document.querySelector('.sort-list.active');

        const newIconClass = anyActive ? 'ph ph-sort-descending' : 'ph ph-sort-ascending';
        
        if (desktopIcon) desktopIcon.className = newIconClass;
        if (mobileIcon) mobileIcon.className = newIconClass;

        // Also update limit icons if applicable
        const limitIcon = document.getElementById('limitHeaderIcon');
        const limitActive = document.querySelector('.limit-list.active');
        if (limitIcon) {
            limitIcon.className = limitActive ? 'ph ph-caret-up' : 'ph ph-caret-down';
        }
    }

    document.addEventListener('click', () => {
        document.querySelectorAll('.sort-list.active, .limit-list.active').forEach(l => l.classList.remove('active'));
        updateSortIcons();
    });

    // =============================================
    // PRODUCTS TOOLBAR (LIMITS & VIEW)
    // =============================================
    const limitHeader = document.getElementById('limitHeader');
    const limitList = document.getElementById('limitList');
    const limitItems = document.querySelectorAll('.limit-list li');
    const currentLimitLabel = document.getElementById('currentLimit');

    if (limitHeader && limitList) {
        limitHeader.addEventListener('click', (e) => {
            e.stopPropagation();
            limitList.classList.toggle('active');
            updateSortIcons(); // Re-use for limit icons
        });
        document.addEventListener('click', () => {
            limitList.classList.remove('active');
            updateSortIcons();
        });
        ...
        limitItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (currentLimitLabel) currentLimitLabel.textContent = item.textContent;
                currentPage = 1;
                renderProducts();
                limitList.classList.remove('active');
                updateSortIcons();
    ...
    viewBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            viewBtns.forEach(b => {
                ...
                if (b.dataset.view === 'grid') icon.className = 'ph ph-squares-four';
                else icon.className = 'ph ph-grid-four';
            });
            
            btn.classList.add('active');
            // Show ph-dots-nine for active button
            const activeIcon = btn.querySelector('i');
            if (activeIcon) activeIcon.className = 'ph ph-dots-nine';
            
            currentLayoutView = btn.dataset.view;
            renderProducts();
        });
    });
    ...
    function openMobileDrawer() {
        ...
        // Mirror desktop sidebar filters to mobile drawer if empty
        if (mobileFilterContent && mobileFilterContent.children.length === 0) {
            const sidebarColumn = document.querySelector('.category-sidebar-column');
            if (sidebarColumn) {
                // Focus on mirroring the main filter blocks
                const filters = sidebarColumn.querySelectorAll('.sidebar-filter-block, .inline-price-filter');
                filters.forEach(f => {
                    const clone = f.cloneNode(true);
                    // Ensure cloned IDs or inputs don't conflict (standard re-bind will handle them)
                    mobileFilterContent.appendChild(clone);
                });
                
                // Re-init mirrored sliders and listeners if needed
                initMobileFilterListeners();
            }
        }
    ...
    if (applyFiltersBtn) applyFiltersBtn.addEventListener('click', closeMobileDrawer);
    
    if (clearFiltersBtn) {
        clearFiltersBtn.addEventListener('click', () => {
             // Dispatch a custom reset if needed, or manually reset inputs
             currentMinPrice = null;
             currentMaxPrice = null;
             window.currentStatusFilters = [];
             window.currentResolutionFilters = [];
             
             // Update UI of inputs
             const inputs = mobileFilterContent.querySelectorAll('input[type="checkbox"]');
    ...
    function initMobileFilterListeners() {
        if (!mobileFilterContent) return;
        
        // Re-attach listeners to cloned inputs in drawer
        mobileFilterContent.addEventListener('change', (e) => {
            if (e.target.name === 'status') {
                const checked = Array.from(mobileFilterContent.querySelectorAll('input[name="status"]:checked')).map(i => i.value);
                window.currentStatusFilters = checked;
            }
            if (e.target.name === 'resolution') {
                const checked = Array.from(mobileFilterContent.querySelectorAll('input[name="resolution"]:checked')).map(i => i.value);
                window.currentResolutionFilters = checked;
            }
            renderProducts();
        });
        
        // Mobile price slider local handling
        const dMin = mobileFilterContent.querySelector('#priceMinSlider');
        const dMax = mobileFilterContent.querySelector('#priceMaxSlider');
        const dMinLabel = mobileFilterContent.querySelector('#priceMinLabel');
        const dMaxLabel = mobileFilterContent.querySelector('#priceMaxLabel');
        const dTrack = mobileFilterContent.querySelector('#rangeTrack');

        if (dMin && dMax) {
            const updateDrawerSlider = () => {
                const min = parseInt(dMin.value);
                const max = parseInt(dMax.value);
                
                if (min > max) dMin.value = max;
                if (max < min) dMax.value = min;

                const finalMin = parseInt(dMin.value);
                const finalMax = parseInt(dMax.value);

                if (dMinLabel) dMinLabel.textContent = finalMin;
                if (dMaxLabel) dMaxLabel.textContent = finalMax;

                if (dTrack) {
                    const rangeMin = parseInt(dMin.min);
                    const rangeMax = parseInt(dMin.max);
                    const left = ((finalMin - rangeMin) / (rangeMax - rangeMin)) * 100;
                    const right = ((finalMax - rangeMin) / (rangeMax - rangeMin)) * 100;
                    dTrack.style.left = left + '%';
                    dTrack.style.right = (100 - right) + '%';
                }

                currentMinPrice = finalMin;
                currentMaxPrice = finalMax;
                renderProducts();
            };

            dMin.addEventListener('input', updateDrawerSlider);
            dMax.addEventListener('input', updateDrawerSlider);
        }
    }
    ...
    if (mobileSortBtn) {
        mobileSortBtn.addEventListener('click', () => {
            // Cycle through sort options or trigger a small bottom sheet?
            // For now, let's open the desktop sort list if reachable or just toggle
            if (sortList) sortList.classList.toggle('active');
        });
    }
